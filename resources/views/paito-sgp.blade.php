...
@section('content')
    ...
    <div class="col-12 d-flex gap-3 d-lg-none">
        ...
        <div class="col">
            <div class="button-gradient text-center">
                <a href="{{ route('paito.hongkong') }}" class="text-white text-decoration-none">
                    PAITO <br> HONGKONG
                </a>
            </div>
        </div>
    ...
    <table id="drawing-table" class="noselect mt-2 mb-3" border="1" width="100%">
        ...
        @php
            ...
            $data_sgp = Singapore::orderByDesc('date')
                ->get()
        ...
        @endphp
        <tbody>
            @foreach ($data_sgp as $week => $items)
                <tr>
                    @php
...
foreach ($items as $item) {
        ...
        case 'Sunday':
            $days['minggu'] = $item;
                    ...
                    @foreach ($days as $day => $item)
                        @if ($item)
                            <td class="asu">{{ substr($item->first_prize, 0, 1) }}</td>
                            <td class="asu">{{ substr($item->first_prize, 1, 1) }}</td>
                            <td class="asu">{{ substr($item->first_prize, 2, 1) }}</td>
                            <td class="asu">{{ substr($item->first_prize, 3, 1) }}</td>
                            <td class="asux">
                                {{ intval(substr($item->first_prize, 2, 1)) + intval(substr($item->first_prize, 3, 1)) < 10
                                    ? intval(substr($item->first_prize, 2, 1)) + intval(substr($item->first_prize, 3, 1))
                                    : intval(
                                            substr(
                                                (string) (intval(substr($item->first_prize, 2, 1)) + intval(substr($item->first_prize, 3, 1))),
                                                0,
                                                1,
                                            ),
                                        ) +
    ...
    <script type="text/javascript" src="/storage/sgp/script-live.js"></script>
    <script type="text/javascript" src="/storage/sgp/script-lite.js"></script>
    <script type="text/javascript" src="/storage/sgp/script-site.js"></script>
