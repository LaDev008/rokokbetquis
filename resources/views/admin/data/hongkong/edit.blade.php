...
<div class="p-4 d-flex flex-column gap-4">
    <h1 class="text-center">Data Hongkong
        {{ Carbon\Carbon::parse($hongkong->date)->isoFormat('dddd, D MMMM YYYY') }}</h1>

    <form action="{{route("admin.data.hongkong.update", $hongkong->id)}}" method="POST">
        @csrf
        @method('put')
        <div class="input-group mb-3">
            <span class="input-group-text" id="firstResult">Tanggal</span>
            <input type="date" class="form-control" aria-label="first_result"
                aria-describedby="firstResult" name='date' value="{{$hongkong->date}}">
        </div>
        <div class="input-group mb-3">
            ...
            <input type="text" class="form-control" placeholder="-" aria-label="first_result"
                aria-describedby="firstResult" name='first_result' required value="{{$hongkong->first_result}}">
        </div>
        <div class="input-group mb-3">
            ...
            <input type="text" class="form-control" placeholder="-" aria-label="second_result"
                aria-describedby="secondResult" name='second_result' required value="{{$hongkong->second_result}}">
        </div>
        <div class="input-group mb-3">
            ...
            <input type="text" class="form-control" placeholder="-" aria-label="third_result"
                aria-describedby="thirdResult" name='third_result' required value="{{$hongkong->third_result}}">
        </div>
